...
function LayoutClashRoyale() {
  ...
  useEffect(() => {
    const loadGameItems = async () => {
      try {
        ...
        const filteredProducts = response.data.filter(product => product.game === "Clash Royale");
  ...
  const filteredProducts = selectedCategory === 'all'
    ? products
    : products.filter(product => product.GameItemDetail.type === selectedCategory);
  ...
  const getRarityClass = (rarity) => {
    switch (rarity) {
      case 'Legendary': return 'cr-legendary-item';
      case 'Epic': return 'cr-epic-item';
      case 'Premium': return 'cr-premium-item';
      case 'Ultra': return 'cr-ultra-item';
      default: return '';
    }
  };
      ...
      <header className="cr-header">
        <div className="cr-header-container">
          <div className="cr-logo">
            <img src="/img/app-icon-clashroyale.png" alt="Clash Royale Logo" className="cr-logo-img" />
            <h1 className="cr-logo-text">CLASH ROYALE МАГАЗИН</h1>
          </div>

          {isAuthenticated ? (
            <div className='clash-app-cs-header-container'>
              <div className="cr-cart-icon" onClick={() => setShowCart(!showCart)}>
      ...
      <main className="cr-main">
        {showCart ? (
          <div className="cr-cart-container">
            <h2><span className="cr-section-icon">🛒</span> Ваша корзина</h2>
            {cart.length === 0 ? (
              <div className="cr-empty-cart">
                ...
                <div className="cr-cart-king">
                  <img src="/api/placeholder/150/150" alt="Грустный Король" className="cr-sad-king" />
                </div>
              </div>
            ) : (
              <>
                <div className="cr-cart-items">
                  {cart.map(item => (
                    <div key={`cart-${item.id}`} className={`cr-cart-item ${getRarityClass(item.GameItem.GameItemDetail.rarity)}`}>
                      <img
                      ...
                      <div className="cr-cart-item-details">
                        ...
                        <div className="cr-item-rarity">{item.GameItem.GameItemDetail.rarity}</div>
                        <p>
                      ...
                      <button className="cr-remove-btn" onClick={() => removeFromCart(item.id)}>✕</button>
                    </div>
                  ))}
                </div>
                <div className="cr-cart-summary">
                  <div className="cr-summary-details">
                    ...
                    <p className="cr-total">Итого: <span className="cr-price-highlight">${calculateTotal()}</span></p>
                  </div>
                  <button className="cr-checkout-btn" onClick={() => navigate("/checkout")}>
                    <span className="brawl-btn-icon" >💰</span> Перейти к оплате
                  </button>
                </div>
              </>
            )}
            <button className="cr-continue-shopping" onClick={() => setShowCart(false)}>
              ...
              {filteredProducts.map(product => (
                ...
                >
                  <div className="cr-item-header">
                    ...
                    <div className="cr-rarity-banner">{product.GameItemDetail.rarity}</div>
                  </div>
                  <div className="cr-item-content">
                    <div className="cr-item-image-container">
                      <img src={`${process.env.REACT_APP_API_URL}${product.imageUrl}`} alt={product.name} className="cr-item-image" />
                      {product.GameItemDetail.type === 'pass' && <div className="cr-special-badge">СПЕЦИАЛЬНО</div>}
                    </div>
                    <p className="cr-item-description">{product.description}</p>
                    <div className="cr-product-price">
                      <span className="cr-price">${product.GameItemDetail.price}</span>
                      <span className="cr-gems"><span className="cr-gem-icon">💎</span> {product.GameItemDetail.gems}</span>
                      <input
                        type="number"
                        min="1"
